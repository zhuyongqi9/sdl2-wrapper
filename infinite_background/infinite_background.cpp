...
public: 
    InfiniteScrollingBackground(SDL_Renderer *renderer, std::string path): renderer(renderer)
    {
    ...
    void move_right() { 
        if (offset > 0) {
            ...
            x += speed; 
            if (x > texture->width - SCREEN_WIDTH) {
                offset = SCREEN_WIDTH;
                x = 0;
            }
        }
    ...
    void render() {
        if (offset > 0) {
            SDL_Rect src1 = {texture->width - offset, y, offset, SCREEN_HEIGHT};
            SDL_Rect dst1 = {0, 0, offset, SCREEN_HEIGHT};
            SDL_RenderCopy(renderer, texture->get(), &src1, &dst1);
            SDL_Rect src2 = {x, y, SCREEN_WIDTH - offset, SCREEN_HEIGHT};
            SDL_Rect dst2 = {offset, 0, SCREEN_WIDTH-offset, SCREEN_HEIGHT};
            SDL_RenderCopy(renderer, texture->get(), &src2, &dst2);
        } else {
...
int main(int argc, char **argv) {
    try {
        ...
        WWindow window("Infinite Background", SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN);
        WRenderer renderer(window.get(), -1, SDL_RENDERER_ACCELERATED);
        
        InfiniteScrollingBackground background(renderer.get(), PRO_DIR + "/infinite_background/bg.png");
        
        Point point(renderer.get(), PRO_DIR + "/infinite_background/dot.bmp");
