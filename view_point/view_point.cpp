...
int main(int argc, char **argv) {
    ...
    while (!quit) {
        ...
        SDL_Rect topLeftViewPoint;
        topLeftViewPoint.x = 0;
        topLeftViewPoint.y = 0;
        topLeftViewPoint.w = SCREEN_WIDTH / 2;
        topLeftViewPoint.h = SCREEN_HEIGHT / 2;
        SDL_RenderSetViewport(gRenderer, &topLeftViewPoint);
        SDL_RenderCopy(gRenderer, gTexture, NULL, NULL);

        SDL_Rect topRightViewPoint;
        topRightViewPoint.x = SCREEN_WIDTH / 2 + 1;
        topRightViewPoint.y = 0;
        topRightViewPoint.w = SCREEN_WIDTH / 2;
        topRightViewPoint.h = SCREEN_HEIGHT / 2;
        SDL_RenderSetViewport(gRenderer, &topRightViewPoint);
        SDL_RenderCopy(gRenderer, gTexture, NULL, NULL);

        SDL_Rect bottomViewport;
        bottomViewport.x = 0;
        bottomViewport.y = SCREEN_HEIGHT/ 2 + 1;
